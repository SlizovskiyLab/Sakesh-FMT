...
merged_mobilome_df['Disease_type'] = merged_mobilome_df['Disease_type'].fillna('Unknown')
...
unique_diseases = merged_mobilome_df['Disease_type'].unique()
...
ax = sns.scatterplot(x='PC1', y='PC2', hue='Disease_type', data=merged_mobilome_df, palette=disease_colors, alpha=0.7, edgecolor='k')

# Computing confidence ellipses
for disease in unique_diseases:
    subset = merged_mobilome_df[merged_mobilome_df['Disease_type'] == disease]
    confidence_ellipse(subset['PC1'], subset['PC2'], ax, disease_colors.get(disease, 'gray'))

plt.xlabel('Principal Component 1')
plt.ylabel('Principal Component 2')
plt.title('PCA of Aitchison Distances for Mobilome Samples (Disease Type)')
