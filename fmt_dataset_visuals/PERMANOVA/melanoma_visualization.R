...
# Collect all results
results <- bind_rows(
  extract_adonis_result(extrac_melanoma_mob, "DNA Extraction", "Mobilome"),
  extract_adonis_result(extrac_melanoma_res, "DNA Extraction", "Resistome"),
  extract_adonis_result(route_melanoma_mob, "FMT Route", "Mobilome"),
  extract_adonis_result(route_melanoma_res, "FMT Route", "Resistome"),
  extract_adonis_result(sequencer_melanoma_mob, "Sequencer", "Mobilome"),
  extract_adonis_result(sequencer_melanoma_res, "Sequencer", "Resistome"),
  extract_adonis_result(study_melanoma_mob, "Study", "Mobilome"),
  extract_adonis_result(study_melanoma_res, "Study", "Resistome"),
  extract_adonis_result(donrec_melanoma_mob, "Donor/Recipient", "Mobilome"),
  extract_adonis_result(donrec_melanoma_res, "Donor/Recipient", "Resistome")
)
...
bar_plot <- ggplot(results, aes(x = Factor, y = R2, fill = DataType)) +
  ...
  labs(y = "Adonis R²", x = NULL, fill = "Melanoma") +
