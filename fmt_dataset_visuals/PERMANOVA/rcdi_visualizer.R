...
# Collect all results
results <- bind_rows(
  extract_adonis_result(prep_rcdi_mob, "FMT Prep", "Mobilome"),
  extract_adonis_result(prep_rcdi_res, "FMT Prep", "Resistome"),
  extract_adonis_result(extrac_rcdi_mob, "DNA Extraction", "Mobilome"),
  extract_adonis_result(extrac_rcdi_res, "DNA Extraction", "Resistome"),
  extract_adonis_result(route_rcdi_mob, "FMT Route", "Mobilome"),
  extract_adonis_result(route_rcdi_res, "FMT Route", "Resistome"),
  extract_adonis_result(sequencer_rcdi_mob, "Sequencer", "Mobilome"),
  extract_adonis_result(sequencer_rcdi_res, "Sequencer", "Resistome"),
  extract_adonis_result(study_rcdi_mob, "Study", "Mobilome"),
  extract_adonis_result(study_rcdi_res, "Study", "Resistome"),
  extract_adonis_result(donrec_rcdi_mob, "Donor/Recipient", "Mobilome"),
  extract_adonis_result(donrec_rcdi_res, "Donor/Recipient", "Resistome")
)
...
bar_plot <- ggplot(results, aes(x = Factor, y = R2, fill = DataType)) +
  ...
  labs(y = "Adonis R²", x = NULL, fill = "rCDI") +
