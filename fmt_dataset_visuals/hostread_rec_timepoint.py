...
df["trimmomatic_input_reads"] = df["trimmomatic_input_reads"] / 1_000_000
...
df_filtered = df.dropna(subset=["trimmomatic_input_reads", "donor_pre_post"])
...
for condition, color in colors.items():
    ...
    plt.scatter(subset["timepoint"], subset["trimmomatic_input_reads"], 
...
for condition, color in colors.items():
    ...
    subset["Smoothed"] = subset["trimmomatic_input_reads"].rolling(window=window_size, center=True).mean()
...
plt.ylabel("Hostreads (millions)")
plt.title("Hostreads Over Time for preFMT and postFMT Samples")
plt.legend()
plt.grid(alpha=0.3)
