...
ax = sns.scatterplot(x='PC1', y='PC2', hue='fmt_prep', data=merged_df, palette='tab10', alpha=0.7, edgecolor='k')
...
for fmt in merged_df['fmt_prep'].unique():
    subset = merged_df[merged_df['fmt_prep'] == fmt]
    if len(subset) < 2:
        continue
    
    mean_x, mean_y = subset['PC1'].mean(), subset['PC2'].mean()
    ci_x, ci_y = sem(subset['PC1']) * 1.96, sem(subset['PC2']) * 1.96 
    color = legend_colors.get(fmt, 'gray') 
    circle = plt.Circle((mean_x, mean_y), max(ci_x, ci_y), color=color, alpha=0.2)
    plt.gca().add_patch(circle)

plt.xlabel('Principal Component 1')
plt.ylabel('Principal Component 2')
plt.title('PCA of Aitchison Distances for Resistome Samples (FMT Prep-based)')
